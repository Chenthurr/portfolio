...
export default function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const update = () => setEnabled(pointer.matches && motion.matches); update();
    pointer.addEventListener?.('change', update); motion.addEventListener?.('change', update);
  ...
  return <div ref={cursorRef} className={`custom-hat-cursor ${active ? 'is-active' : ''}`} aria-hidden="true"><StrawHat className="custom-hat-cursor-art" /><span className="custom-hat-cursor-dot" /></div>;
}
